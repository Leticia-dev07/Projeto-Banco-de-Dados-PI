...
-- obs: READS SQL DATA é uma cláusula de característica do MySQL que define o comportamento da função, ou seja apenas le os dados do banco não modifica(insert, update e etc..)
delimiter $$
...
DELIMITER ;
-- 3°
-- stored procedure que permite cadastrar um novo cliente no sistema, incluindo seus dados pessoais, endereço e telefone. 
-- A procedure verificará se o telefone e endereço já existem antes de criar novos registros.
DELIMITER //
...
BEGIN
    ...
    ELSE
        ...
        SET p_mensagem = 'Cliente cadastrado com sucesso! ID: ' || v_idCliente;
    END IF;
END //
DELIMITER ;
