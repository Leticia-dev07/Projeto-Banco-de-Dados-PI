...
SELECT 
    s.nomeComum AS Tipo_Semente,
    COUNT(DISTINCT l.idLote) AS Qtd_Lotes,
    SUM(e.quantSaida) AS Total_Saida_Nacional,
    SUM(l.quantReceb) AS Total_Recebido,
    ROUND(AVG(l.precUnit), 2) AS Preco_Medio
FROM Estoque e
...
GROUP BY s.idSementes, s.nomeComum
ORDER BY Total_Saida_Nacional DESC;
